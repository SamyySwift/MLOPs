...
def prep_db():
    with psycopg.connect(
        "host=localhost port=5432 user=postgres password=1234", autocommit=True
    ) as conn:
        res = conn.execute("SELECT 1 FROM pg_database WHERE datname='test'")
        if len(res.fetchall()) == 0:
            conn.execute("create database test;")
        with psycopg.connect(
            "host=localhost port=5432 dbname=test user=postgres password=1234"
        ) as conn:
            conn.execute(create_table_statement)


@task(name="calulate metrics", retries=2, retry_delay_seconds=5)
def calculate_metrics_postgresql(curr, i):
    ...
    fare_amount_quantile = result["metrics"][1]["result"]["reference"]["value"]
    ...
    curr.execute(
        "insert into dummy_metrics(timestamp, prediction_drift, fare_amount_quantile, num_drifted_columns, share_missing_values) values (%s, %s, %s, %s, %s)",
        (
